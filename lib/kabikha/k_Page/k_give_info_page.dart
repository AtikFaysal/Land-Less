...
class KabikhaGiveInfoPage extends StatefulWidget {
  LoginResponse loginResponse;

  KabikhaGiveInfoPage(this.loginResponse);

  @override
  _KabikhaGiveInfoPageState createState() => _KabikhaGiveInfoPageState(loginResponse);
}

class _KabikhaGiveInfoPageState extends State<KabikhaGiveInfoPage> {
  LoginResponse loginResponse;

  _KabikhaGiveInfoPageState(this.loginResponse);
  ...
  Widget _submitButton(BuildContext context) {
    return FlatButton(
        onPressed: ()async {
          ...
          ApiCall().KabikhaProjectEntry(task).then((value) {
            if(value['status']==200){
              setState(() {
                clearData();
                _isLoading=false;
