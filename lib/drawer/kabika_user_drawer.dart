...
import 'package:land_less/utils/kabikha_user_menu.dart';
...
class KabikhaUserDrawer extends StatefulWidget {
  LoginResponse loginResponse;

  KabikhaUserDrawer(this.loginResponse);

  @override
  _KabikhaUserDrawerState createState() => _KabikhaUserDrawerState(loginResponse);
}

class _KabikhaUserDrawerState extends State<KabikhaUserDrawer> {
  LoginResponse loginResponse;

  _KabikhaUserDrawerState(this.loginResponse);
  int selectedMenuItemId;
  String pageName='ড্যাশবোর্ড';

  @override
  void initState() {
    selectedMenuItemId = kabikhaMenuWithIcon.items[0].id;
    super.initState();
  }
      ...
      child: new MaterialApp(
        ...
        home: DrawerScaffold(
          ...
          builder: (context, id) => IndexedStack(
            index: id,
            children: kabikhaMenuWithIcon.items
                .map((e) {
              if(e.id==0){
                return KDashboard(loginResponse);
              }else if(e.id==1){
                return KabikhaGiveInfoPage(loginResponse);
              }
              else if(e.id==2){
  ...
  headerTitle(dynamic id){
    switch(id){
      case 0:
        ...
        break;
      case 1:
        pageName="কাবিখা প্রকল্প তথ্য দিন";
        break;
      case 2:
