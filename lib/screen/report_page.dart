...
import 'package:land_less/screen/report_details.dart';
...
class ReportPage extends StatefulWidget {
  ...
  ReportPage(this.loginResponse);

  @override
  _ReportPageState createState() => _ReportPageState(loginResponse);
}

class _ReportPageState extends State<ReportPage> {
  LoginResponse loginResponse;

  _ReportPageState(this.loginResponse);
  ...
  List<String> _category = ["গৃহহীন", "ভূমিহীন"];
  String _slectCategory;
  List<String> _status = ["অনুমোদিত","ঘর হস্তান্তরিত", "বাতিলকৃত"];
  String _selectStatus;
  List<MemberData>_member=[];
  void loadPublicData(){
    setState(() {
      _isLoading=true;
    });
  ...
  void loadPuMemberData(BuildContext context,Map<String,dynamic>task)async{
    ApiCall().getAccordingToReportMemberById(task).then((value) {
      if(value!=null){
        _member.clear();
        if(value.status==200){
          if(value.memberData.isNotEmpty){
            for(int i=0;i<value.memberData.length;i++){
              setState(() {
                _member.add(value.memberData[i]);
                _isLoading=false;
              });
            }
          }
          ...
          onPressed: (){
            ...
            if(_selectStatus!=null){
              if(_selectStatus.trim()=='অনুমোদিত'){
                status='2';
              }else if(_selectStatus.trim()=='ঘর হস্তান্তরিত'){
                status='3';
              }else{
                status='0';
              }
            ...
            if(_financialYearId!=null){
              Map<String,dynamic>task={
                ...
                'type':_slectCategory!=null?_slectCategory.trim()=='গৃহহীন'?"1":"2":'',
    ...
    return  Padding(
      ...
      child: Container(
        ...
        child: TypeAheadFormField<PublicUpData>(
          ...
          validator: (value) =>
          value.isEmpty ? 'ইউনিয়ন/পৌরসভা নাম আবশ্যক' : null,
          //onSaved: (value) => this._selectedCity = value,
        ),
      ),
    );
                ...
                child: Container(
                  ...
                  child: DropdownButton<String>(
                    ...
                    hint: Padding(padding: EdgeInsets.only(left: 10),
                      child: Text('উপকারভোগী ধরন নির্বাচন করুন',style: TextStyle(fontWeight: FontWeight.w300,fontSize: 12),),),
  ...
  Widget _buildListTile(MemberData memberData){
    ...
    return GestureDetector(
      onTap: ()=>Navigator.of(context).push(MaterialPageRoute(builder: (context)=>HouseReportDetails(memberData.memberId,loginResponse))),
      child: Container(
        decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(7.0),
            color: Colors.white,
