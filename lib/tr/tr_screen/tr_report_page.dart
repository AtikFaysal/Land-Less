...
import 'package:land_less/tr/tr_screen/tr_report_details.dart';
import 'package:land_less/utils/Constant.dart';
import 'package:land_less/widget/toast_massage.dart';
class TrReportPage extends StatefulWidget {
  LoginResponse loginResponse;

  TrReportPage(this.loginResponse);

  @override
  _TrReportPageState createState() => _TrReportPageState(loginResponse);
}

class _TrReportPageState extends State<TrReportPage> {
  LoginResponse loginResponse;

  _TrReportPageState(this.loginResponse);
  ...
  List<TrProjectData>_trReportData=[];
  void loadPublicData(){
    setState(() {
      _isLoading=true;
    });
  ...
  void loadPuMemberData(BuildContext context,Map<String,dynamic>task)async{
    TrApi().getTrProjectReportById(task).then((value) {
      if(value!=null){
        _trReportData.clear();
        if(value.status==200){
          if(value.trProjectData.isNotEmpty){
            for(int i=0;i<value.trProjectData.length;i++){
              setState(() {
                _trReportData.add(value.trProjectData[i]);
                _isLoading=false;
              });
            }
          }
      ...
      body: ListView(
        children: [
          _isLoading?Center(child: CircularProgressIndicator(),):Container(
            height: loginResponse.response.userRole=='1'?MediaQuery.of(context).size.height*.8/2:MediaQuery.of(context).size.height*.6/2,
            child: Column(
          ...
          _trReportData.isNotEmpty?Container(
            height:double.parse(_trReportData.length.toString())*225.0,
  ...
  Widget listMember(BuildContext context){
    return Padding(
      ...
      child: ListView.separated(
        ...
        itemCount: _trReportData.length,
        separatorBuilder: (context,int i)=>Divider(),
        itemBuilder: (context,int i){
          return _buildListTile(_trReportData[i]);
        },
      ),
    );
  }
  Widget _buildListTile(TrProjectData memberData){

    return GestureDetector(
      onTap: ()=>Navigator.of(context).push(MaterialPageRoute(builder: (context)=>TrReportDetails(loginResponse,memberData.trProjectId))),
      child: Container(
        decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(7.0),
            color: Colors.white,
